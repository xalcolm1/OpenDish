...
const Stars = props => {
    ...
    const {targetRating, onClick} = props
    let rating = [];

    for(let i = 1; i <= 5; i ++){
        let nextStar
        if(i < targetRating && targetRating < (i + 1)){
            let percent = Math.floor((targetRating - i) * 100) 
            ...
            className={ i <= targetRating ? "filledStar" : "emptyStar" }
            key={ i }
            >
